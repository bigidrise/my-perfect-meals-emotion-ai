...
export type MacroNumbers = { protein?: number; carbs?: number; fat?: number; calories?: number };
...
export function sumMacroNumbers(items: MacroNumbers[]) {
  const tot = items.reduce((a, x) => ({
    ...
    carbs:   (a.carbs || 0) + Number(x.carbs || 0),
    fat:     (a.fat || 0) + Number(x.fat || 0),
    calories:(a.calories || 0) + Number(x.calories || 0),
  }), { protein:0, carbs:0, fat:0, calories:0 } as MacroNumbers);
  return roundTotals(tot);
}

export function formatClipboard(m: {protein:number; carbs:number; fat:number; calories:number}) {
