export async function logMeal(mealInstanceId: string, body: any = {}) {
  const r = await fetch(`/api/meals/${mealInstanceId}/log`, {
    method: 'POST', 
...
export async function skipMeal(mealInstanceId: string) {
  const r = await fetch(`/api/meals/${mealInstanceId}/skip`, { 
    method: 'POST' 
  });
...
export async function replaceAndOptionalLog(mealInstanceId: string, body: any) {
  const r = await fetch(`/api/meals/${mealInstanceId}/replace-and-optional-log`, {
    method: 'POST', 
    headers: { 'Content-Type': 'application/json' }, 
    body: JSON.stringify(body)
  });
