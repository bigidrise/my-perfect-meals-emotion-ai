...
export async function generateFridgeRescueMeals(request: FridgeRescueRequest): Promise<FridgeRescueMeal[]> {
  const { fridgeItems, user } = request;
  ...
  "meals": [
    {
      ...
      "ingredients": [{"name": "ingredient from fridge", "quantity": "realistic amount", "unit": "tbsp/cup/etc"}],
      "instructions": "Step-by-step cooking instructions as single string",
      "calories": number (200-500 range),
      "protein": number (10-40 grams),
      "carbs": number (15-50 grams), 
      "fat": number (5-25 grams),
